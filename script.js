...
function spawnEnemy(bound) {
    if (enemies.length < 5) {
        enemies.push(new Enemy(canvas, {x: Math.random()*(bound.w)+bound.x, y: Math.random()*(bound.h)+bound.y}));
    }
}
...
let tank = new Tank(canvas, {x: 100, y: canvas.canvas.height/2-75});
// Autospawn an enemy
spawnEnemy({x: 300, y: 0, w: canvas.canvas.width, h: canvas.canvas.height});
// Main interval
setInterval(cycle, 10);

// Enemy spawner interval
setInterval(() => spawnEnemy({x: 300, y: 0, w: canvas.canvas.width, h: canvas.canvas.height}), 5000);
